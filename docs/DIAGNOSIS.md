...
- **S2 — All integration credentials are plaintext and RLS-readable by any workspace member.** `integrations.access_token` holds raw Stripe `sk_`/`rk_` keys, GHL API keys, and Meta OAuth tokens; the RLS SELECT policy grants read to any `is_workspace_member` (`migration 20260707213952:35`). A viewer-role user can pull every client's Stripe secret key via PostgREST. Same for webhook signing secrets (`lead-webhooks.functions.ts:141`).
